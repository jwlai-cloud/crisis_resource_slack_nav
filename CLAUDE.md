...
- **Slack agent** (repo root) — Bolt for Python app on the native agent surface, scaffolded via the Slack CLI (`slack create agent`). The Slack CLI **owns the app's file layout** — do not invent app paths or entry points before that scaffold exists; adopt whatever structure it generates, then record it here.
...
└── <app files>                # generated by `slack create agent` — layout TBD until W1 scaffold
```
...
| `make run` | `slack run` against the sandbox *(verify command after W1 scaffold)*. |
...
Slack CLI commands (`slack create agent`, `slack run`, auth/deploy flags) — **fill in the real invocations here after the W1 scaffold**; placeholders must not be invented before then.
